...
def add_noise(mel_spectrogram_db, noise_factor=0.005):
    noise = np.random.normal(0, noise_factor, mel_spectrogram_db.shape)
    return mel_spectrogram_db + noise



def plot_spectrogram(signal, output_path, interval_index, f, file_name, classification, freqmask):
    # Compute the Mel spectrogram
    mel_spectrogram = librosa.feature.melspectrogram(y=signal, sr=44100, n_mels=128, fmax=8000)
    mel_spectrogram_db = librosa.power_to_db(mel_spectrogram, ref=np.max)

    if freqmask:
        mel_spectrogram_db = add_noise(mel_spectrogram_db)
...
def main(file_path, file_name, classification):
    ...
    f = open("MEL_Spectograms_Noise/labels.csv", "a")
    # Trim silence
    ...
    for i in range(num_intervals):
        ...
        apply_mask = random.random() < 0.3  # 50% chance of time masking
        if apply_mask:
            plot_spectrogram(interval_signal, pl.Path('MEL_Spectograms_Noise/images'), i + 1, f, file_name, classification,1)
        else:
            plot_spectrogram(interval_signal, pl.Path('MEL_Spectograms_Noise/images'), i + 1, f, file_name, classification,0)
