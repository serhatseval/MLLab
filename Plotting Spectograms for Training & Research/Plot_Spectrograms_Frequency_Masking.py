...
def frequency_masking(mel_spectrogram_db, max_mask_percentage=0.2):
    freq_bins = mel_spectrogram_db.shape[0]
    mask_size = int(freq_bins * max_mask_percentage)
...
def plot_spectrogram(signal, output_path, interval_index, f, file_name, classification, freqmask):
    ...
    if freqmask:
        mel_spectrogram_db = frequency_masking(mel_spectrogram_db)
...
def main(file_path, file_name, classification):
    ...
    f = open("MEL_Spectograms_FreqMasking_20/labels.csv", "a")
    # Trim silence
    ...
    for i in range(num_intervals):
        ...
        if apply_mask:
            plot_spectrogram(interval_signal, pl.Path('MEL_Spectograms_FreqMasking_20/images'), i + 1, f, file_name, classification,1)
        else:
            plot_spectrogram(interval_signal, pl.Path('MEL_Spectograms_FreqMasking_20/images'), i + 1, f, file_name, classification,0)
